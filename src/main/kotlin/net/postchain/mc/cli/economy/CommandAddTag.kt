...
import net.postchain.mc.cli.util.priceOption
...
class CommandAddTag : ECBaseCommand(
    ...
    private val scuPrice by priceOption("-scup", "--scu-price", help = "SCU price in USD per day.")
    private val extraStoragePrice by priceOption("-esp", "--extra-storage-price", help = "Extra storage price in USD per day.")
    ...
    override fun runEC(client: PostchainClient, economyChainClient: PostchainClient) {
        ...
        when {
            ...
            ecVersion.version < ECONOMY_CHAIN_REQUIRE_PROVIDER_IDENTIFIER_AND_DYNAMIC_CU_VERSION -> {
                transactionBuilder()
                        .createTagOperationV57(name, scuPrice.times(UNITS_PER_USD.toBigDecimal()).toLong(),
            ...
            ecVersion.version < ECONOMY_CHAIN_TAG_COMPUTE_REQUEST_PRICE_VERSION -> {
                transactionBuilder()
                        .createTagOperationECV66(clientProviderPubkey, name,
                ...
                transactionBuilder()
                        .createTagOperation(clientProviderPubkey, name,
                                scuPrice.times(UNITS_PER_USD.toBigDecimal()).toLong(),
                                extraStoragePrice.times(UNITS_PER_USD.toBigDecimal()).toLong(),
                                extraComputeRequestPrice?.times(UNITS_PER_USD.toBigDecimal())?.toLong() ?: 0)
            }
        }
                .postOrSave()
                .printResult(
