...
class CommandAddTagIT : ECTestBase() {
    ...
    fun `add tag with dollar - legacy EC version`(@TempDir dir: Path) =
        ecRestApiTest(dir, ECONOMY_CHAIN_EC_STAKING_REQ_AND_USD_MINOR_UNITS_VERSION - 1) { ecModel ->
            TestProcess.Builder("economy", "add-tag", "--name", "t1", "--scu-price", "1", "--extra-storage-price", "2")
                    ...
                    .start { _ ->
                        ecModel.assertSingleOp("create_tag", listOf(gtv("t1"), gtv(1), gtv(2)))
                    }
        }

    @Test
    fun `add tag with minor units - legacy EC version`(@TempDir dir: Path) =
        ecRestApiTest(dir, ECONOMY_CHAIN_EC_STAKING_REQ_AND_USD_MINOR_UNITS_VERSION - 1) { _ ->
    ...
    fun `add tag with dollar and cent - new EC version`(@TempDir dir: Path) =
            ecRestApiTest(dir, ECONOMY_CHAIN_EC_STAKING_REQ_AND_USD_MINOR_UNITS_VERSION) { ecModel ->
                TestProcess.Builder("economy", "add-tag", "--name", "t1", "--scu-price", "1.234", "--extra-storage-price", "5.123456789")
                        ...
                        .setWorkingDir(dir.toFile())
                        .start { _ ->
                            ecModel.assertSingleOp("create_tag", listOf(gtv("t1"), gtv(1234000), gtv(5123456)))
                        }
            }
}