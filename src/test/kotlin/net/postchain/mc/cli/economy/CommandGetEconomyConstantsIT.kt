    ...
    fun `list constants - legacy EC version`(@TempDir dir: Path) {
        ManagedRestTestApi(dir, ecVersion = ECONOMY_CHAIN_EC_STAKING_REQ_AND_USD_MINOR_UNITS_VERSION - 1)
                .testCommand(CommandGetEconomyConstants()) { result, _ ->
                    ...
                    assertLineValue(result.stdout, "Total_cost_system_providers_in_USD", "800123456")
                    ...
                    assertLineValue(result.stdout, "Staking_requirements_system_provider_own", "1123456")
                    assertLineValue(result.stdout, "Staking_requirements_system_provider_total", "2123456")
                    assertLineValue(result.stdout, "Staking_requirements_dapp_provider_own", "3123456")
                    assertLineValue(result.stdout, "Staking_requirements_dapp_provider_total", "4123456")
                }
    }
    ...
    fun `list constants - new EC version`(@TempDir dir: Path) {
        ManagedRestTestApi(dir, ecVersion = ECONOMY_CHAIN_EC_STAKING_REQ_AND_USD_MINOR_UNITS_VERSION)
                .testCommand(CommandGetEconomyConstants()) { result, _ ->
                    ...
                    assertLineValue(result.stdout, "Staking_requirements_enabled", "true")
                    assertLineValue(result.stdout, "Staking_requirements_stop_payout_days", "14")
                    assertLineValue(result.stdout, "Staking_requirements_system_provider_own", "1.123456")
                    assertLineValue(result.stdout, "Staking_requirements_system_provider_total", "2.123456")
                    assertLineValue(result.stdout, "Staking_requirements_dapp_provider_own", "3.123456")
                    assertLineValue(result.stdout, "Staking_requirements_dapp_provider_total", "4.123456")
                }
    }

    @Test
    fun `list constants - dynamic staking reward share`(@TempDir dir: Path) {
